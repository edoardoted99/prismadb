...
def train_sae_run(run_id: int):
    ...
    try:
        ...
        # 7. Sparsity Metrics
        print(f"[Run #{run.id}] Computing sparsity metrics...")
        model.eval()
        with torch.no_grad():
            # Sample up to 10k for efficiency
            n_sample = min(10000, len(X_std))
            X_sample = X_std[:n_sample].to(device)
            _, _, h_topk = model(X_sample)
            h_cpu = h_topk.cpu()

            # Mean L0: average number of active neurons per input
            l0_per_sample = (h_cpu > 0.0001).float().sum(dim=1)
            mean_l0 = l0_per_sample.mean().item()

            # Dead neurons: fraction that never activate across the sample
            ever_active = (h_cpu > 0.0001).any(dim=0)
            dead_ratio = 1.0 - ever_active.float().mean().item()

            # Sparsity index: 1 - mean_l0/d_latent (1.0 = maximally sparse)
            sparsity_idx = 1.0 - (mean_l0 / d_latent)

        run.mean_l0 = round(mean_l0, 2)
        run.dead_neuron_ratio = round(dead_ratio, 4)
        run.sparsity_index = round(sparsity_idx, 6)
        print(f"[Run #{run.id}] Sparsity: L0={mean_l0:.1f}, dead={dead_ratio*100:.1f}%, index={sparsity_idx:.4f}")

        # 8. Analisi e Salvataggio
